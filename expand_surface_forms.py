            ...
            for value in values.keys():
                ...
                for variant in variants:
                    ...
                    if words[0][2].startswith('V'):
                        infls = [{'person': '2'}, {'person': '-'}] # keep infinitive as well
                    else:
                        infls = [{'case': '1'}, {'case': '2'}, {'case': '3'},
                                 {'case': '4'}, {'case': '6'}, {'case': '7'}]
                        # all numbers for some cases (domain specific)
                        if re.search(r'(^| )(snídaně|oběd|večeře|brunch)( |$)', variant):
                            new_infls = []
                            for infl in infls:
                                for number in ['S', 'P']:
                                    new_infl = infl.copy()
                        ...
                        if slot != 'street' and all(re.match('^[AD]', word[2]) for word in words):
                            new_infls = []
                            for infl in infls:
